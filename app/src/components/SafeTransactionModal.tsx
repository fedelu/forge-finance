...
export const SafeTransactionModal: React.FC<SafeTransactionModalProps> = ({ isOpen, onClose, crucibleId }) => {
  ...
  const handleDeposit = async () => {
    ...
    try {
      console.log('=== SAFE TRANSACTION START ===');
      ...
      // Create a safe transaction - send to a valid address instead of burn
      // Using a well-known devnet address that accepts transfers
      const recipientAddress = new PublicKey('11111111111111111111111111111112'); // System program address (safer)
      
      const transaction = new Transaction();
      
      // Add transfer instruction with a small amount to avoid simulation issues
      const transferAmount = Math.floor(parseFloat(amount) * 1e9);
      ...
      console.log('=== SAFE TRANSACTION SUCCESS ===');
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg p-6 w-full max-w-md">
        <h2 className="text-xl font-bold text-white mb-4">🔥 Safe Transaction - Crucible {crucibleId}</h2>
        ...
        <div className="space-y-4">
          ...
          <div className="p-3 bg-green-900/30 border border-green-600 rounded-lg">
            <p className="text-green-300 text-sm">
              ✅ <strong>SAFE VERSION:</strong> This version uses a safe recipient address to avoid simulation errors.
            </p>
          </div>

          <div className="p-3 bg-blue-900/30 border border-blue-600 rounded-lg">
            <p className="text-blue-300 text-sm">
              ℹ️ <strong>REAL TRANSACTION:</strong> This will use your actual SOL from devnet and send it to a valid address.
            </p>
          </div>
