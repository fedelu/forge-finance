...
export function FogoSessionsButton() {
  ...
  const handlePhantomConnect = async () => {
    ...
    try {
      await connect();
      // Simulate fetching real balance from session
      const mockBalance = Math.random() * 5000 + 100; // Random balance between 100-5100
      setFogoBalance(mockBalance);
    ...
    switch (flowStep) {
      ...
      case 'complete':
        if (isEstablished && sessionData) {
          ...
          // Header integration - compact button when closed
          if (!isOpen) {
            return (
              <button
          ...
          // Full wallet interface when open - positioned relative to header
          return (
